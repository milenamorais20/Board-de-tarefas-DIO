...
public class MainMenu {
    ...
    private void listAllBoards() throws SQLException {
        try(var connection = getConnection()){
            var boards = new BoardQueryService(connection).findAllBoards();
            if (boards.isEmpty()) {
                System.out.println("Nenhum board encontrado. Crie um primeiro!");
                return;
            }
            ...
            for (var board : boards) {
                System.out.printf("%-5d | %-30s%n", board.getId(), board.getName());
            }
            System.out.println("========================\n");
        }
    }
